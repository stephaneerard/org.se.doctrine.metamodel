...
public class SchemeSwitch<T> extends Switch<T> {
	...
	protected T doSwitch(int classifierID, EObject theEObject) {
		switch (classifierID) {
			case SchemePackage.SCHEME: {
				Scheme scheme = (Scheme)theEObject;
				T result = caseScheme(scheme);
				if (result == null) result = caseNamedElement(scheme);
				if (result == null) result = caseAbstractContainer(scheme);
				if (result == null) result = defaultCase(theEObject);
				return result;
			}
			case SchemePackage.GLOBALS: {
				Globals globals = (Globals)theEObject;
				T result = caseGlobals(globals);
				if (result == null) result = defaultCase(theEObject);
				return result;
			}
			case SchemePackage.OPTION: {
